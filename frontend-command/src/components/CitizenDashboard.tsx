...
import { AlertTriangle, MapPin, Bell, Navigation, Globe, Phone, ChevronRight, Shield, LogOut, ArrowLeft, Volume2, Siren, BarChart3, Users, Loader2, Camera, WifiOff, Droplets, Heart, Send, Radio, TrendingUp } from "lucide-react";
...
type View = "home" | "evacuate" | "shelters" | "sos" | "alert-family" | "state-analysis" | "district-detail" | "vulnerability" | "dams" | "report" | "analytics" | "location-info";
...
export default function CitizenDashboard({ onLogout, userState, userDistrict }: Props) {
    ...
    useEffect(() => {
        getSimLocation().then(loc => { setSimLoc(loc); setTowers(getNearestTowers(loc.lat, loc.lon)); });
    }, []);

    const languages = Object.keys(LANG_MAP);
    const highRiskDistricts = getHighRiskDistricts().slice(0, 5);
    const dangerDams = getDangerDams();
    const myDams = (userDistrict.nearbyDams || []).map(d => ({ ...d, district: userDistrict.name, state: userState.name }));
    const myReports = getReports().filter(r => r.state === userState.name);
    const allDams = getAllDams();
    const topVuln = getVulnerableDistricts().slice(0, 5);
            ...
            <div className="bg-[#1a237e] text-white px-4 py-2.5">
                <div className="max-w-lg mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={onLogout} className="text-[10px] bg-white/10 px-2 py-1 rounded flex items-center gap-1 hover:bg-white/20"><LogOut className="w-3 h-3" /> {t(language, "logout")}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "evacuation_route")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
                <div className="bg-white border border-gray-200 rounded-lg p-5"><h2 className="text-base font-bold text-gray-800 mb-1">🧭 {t(language, "safe_route")}</h2><p className="text-sm text-gray-500 mb-4">{t(language, "avoid_flooded")}</p>
                    <div className="space-y-2">{[{ s: 1, d: "Head North on NH-44", dist: "1.2 km", t: "8 min", ok: true }, { s: 2, d: "Turn Right onto Elevated Flyover", dist: "0.8 km", t: "5 min", ok: true }, { s: 3, d: "⚠️ Avoid Main Street (Flooded)", dist: "—", t: "—", ok: false }, { s: 4, d: `Continue to ${userDistrict.name} Relief Camp`, dist: "2.1 km", t: "12 min", ok: true }].map(r => <div key={r.s} className={`flex items-center gap-3 p-3 rounded border ${r.ok ? "bg-gray-50 border-gray-200" : "bg-red-50 border-red-200"}`}><div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ${r.ok ? "bg-[#1a237e]" : "bg-red-500"}`}>{r.s}</div><div className="flex-1"><p className={`text-sm font-medium ${r.ok ? "text-gray-800" : "text-red-700"}`}>{r.d}</p><p className="text-[10px] text-gray-400">{r.dist}·{r.t}</p></div></div>)}</div>
                </div>
    ...
    if(view === "shelters") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "nearby_shelters")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-3">{[{ n: `${userDistrict.name} Relief Camp`, d: "2.1 km", c: "500", a: "187", s: "Open" }, { n: "Community Hall - Block A", d: "3.4 km", c: "300", a: "92", s: "Open" }, { n: "Stadium Emergency Shelter", d: "5.8 km", c: "1200", a: "640", s: "Open" }, { n: "Temple Complex Shelter", d: "1.8 km", c: "150", a: "0", s: "Full" }].map((sh, i) => <div key={i} className={`bg-white border rounded-lg p-4 ${sh.s === "Full" ? "border-red-200 opacity-60" : "border-gray-200"}`}><div className="flex items-center justify-between mb-2"><h3 className="text-sm font-bold text-gray-800">{sh.n}</h3><span className={`text-[10px] px-2 py-0.5 rounded font-bold border ${sh.s === "Full" ? "bg-red-50 text-red-600 border-red-200" : "bg-green-50 text-green-700 border-green-200"}`}>{sh.s}</span></div><div className="flex gap-4 text-xs text-gray-500"><span>📍 {sh.d}</span><span>👥 {sh.a}/{sh.c}</span></div>{sh.s !== "Full" && <button onClick={() => setView("evacuate")} className="mt-3 w-full text-xs font-bold bg-[#1a237e] text-white py-2 rounded hover:bg-[#283593]">Navigate Here →</button>}</div>)}</div>
        </div>
    );
    ...
    if(view === "alert-family") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "alert_family")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">{!familyAlerted ? <><p className="text-sm text-gray-600">Send your safety status via SMS from {userDistrict.name}, {userState.name}.</p><div className="space-y-2">{[{ n: "Mom", p: "+91 98765 XXXXX" }, { n: "Dad", p: "+91 98764 XXXXX" }, { n: "Brother", p: "+91 87654 XXXXX" }].map((c, i) => <div key={i} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-4 py-3"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center"><Users className="w-4 h-4 text-[#1a237e]" /></div><div><p className="text-sm font-semibold text-gray-800">{c.n}</p><p className="text-[10px] text-gray-400">{c.p}</p></div></div><span className="text-[10px] text-gray-400">✓</span></div>)}</div><button onClick={() => { setFamilyAlerted(true); speak(`${t(language, "alert_family")} sent`, language) }} className="w-full py-2.5 bg-[#1a237e] text-white rounded-lg text-sm font-bold">📩 Send Safety Alert (SMS + GPS)</button></> : <div className="text-center space-y-4"><div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-green-100 border border-green-300"><Shield className="w-7 h-7 text-green-600" /></div><h2 className="text-lg font-bold text-green-800">Family Alerted ✓</h2><p className="text-sm text-gray-600">SMS with GPS sent from {userDistrict.name} to 3 contacts.</p><button onClick={() => { setFamilyAlerted(false); setView("home") }} className="w-full py-2.5 bg-[#1a237e] text-white rounded-lg text-sm font-bold">{t(language, "back")}</button></div>}</div>
        </div>
    );

    // ─── VULNERABILITY ───
    if(view === "vulnerability") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "vulnerable_areas")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-3">
                <div className="bg-[#fff3cd] border border-[#ffc107] rounded px-3 py-2 text-xs text-[#856404]"><strong>Priority Notice:</strong> Higher vulnerability = earlier evacuation alerts.</div>
                {/* Show user's own district first */}
                <div className="bg-white border-2 border-[#1a237e] rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2"><div><h3 className="text-sm font-bold text-[#1a237e]">📍 {userDistrict.name} (Your Location)</h3><p className="text-[10px] text-gray-400">{userState.name}</p></div><div className="text-right"><span className="text-sm font-bold text-red-600">{userDistrict.vulnerability.vulnerability_score}/10</span><p className="text-[9px] text-gray-400">Vuln. Score</p></div></div>
                ...
                {topVuln.filter(d => d.name !== userDistrict.name).slice(0, 4).map((d, i) => <div key={i} className="bg-white border border-gray-200 rounded-lg p-4"><div className="flex items-center justify-between mb-2"><div><h3 className="text-sm font-bold text-gray-800">{d.name}</h3><p className="text-[10px] text-gray-400">{(d as any).stateName}</p></div><span className="text-sm font-bold text-red-600">{d.vulnerability.vulnerability_score}/10</span></div><div className="grid grid-cols-4 gap-1 text-[10px]"><div className="bg-gray-50 rounded p-1.5 text-center"><span className="text-gray-400">👴</span><br /><span className="font-bold">{d.vulnerability.elderly_pct}%</span></div><div className="bg-gray-50 rounded p-1.5 text-center"><span className="text-gray-400">👶</span><br /><span className="font-bold">{d.vulnerability.children_pct}%</span></div><div className="bg-gray-50 rounded p-1.5 text-center"><span className="text-gray-400">♿</span><br /><span className="font-bold">{d.vulnerability.disabled_pct}%</span></div><div className="bg-gray-50 rounded p-1.5 text-center"><span className="text-gray-400">💰</span><br /><span className="font-bold">{d.vulnerability.ews_pct}%</span></div></div></div>)}
            </div>
        </div>
    );
    ...
    // ─── DAMS ───
    if(view === "dams") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "dam_monitoring")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-3">
                {myDams.length > 0 && <div className="bg-blue-50 border border-blue-200 rounded p-2 text-xs text-blue-800"><strong>📍 Near {userDistrict.name}:</strong> {myDams.map(d => d.name).join(", ")}</div>}
                {allDams.map((d, i) => <div key={i} className={`bg-white border rounded-lg p-4 ${myDams.some(m => m.name === d.name) ? "border-[#1a237e] border-2" : "border-gray-200"}`}><div className="flex items-center justify-between mb-2"><div><h3 className="text-sm font-bold text-gray-800">{d.name}{myDams.some(m => m.name === d.name) ? " 📍" : ""}</h3><p className="text-[10px] text-gray-400">River: {d.river} · {d.district}, {d.state}</p></div><span className={`text-[10px] px-2 py-0.5 rounded font-bold border ${damC(d.status)}`}>{d.status}</span></div><div className="flex items-center gap-2 mt-2"><div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden"><div className={`h-full rounded-full ${d.current_level_pct > 90 ? "bg-red-500" : d.current_level_pct > 80 ? "bg-yellow-500" : "bg-green-500"}`} style={{ width: `${d.current_level_pct}%` }} /></div><span className="text-xs font-bold text-gray-700">{d.current_level_pct}%</span></div><p className="text-[10px] text-gray-400 mt-1">Capacity: {d.capacity_mcm} MCM</p></div>)}
            </div>
        </div>
    );
    ...
    if(view === "report") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "report_flood")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">{!reportSent ? <><div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3"><p className="text-sm text-gray-600">Report from <strong>{userDistrict.name}, {userState.name}</strong>. GPS auto-attached.</p><div><label className="block text-xs font-bold text-gray-500 mb-1">Type</label><select value={reportType} onChange={e => setReportType(e.target.value as any)} className="w-full px-3 py-2 border border-gray-300 rounded text-sm text-gray-800 focus:border-[#1a237e]"><option value="waterlogging">🌊 Waterlogging</option><option value="drainage_blocked">🚰 Drainage Blocked</option><option value="road_flooded">🛣️ Road Flooded</option><option value="embankment_breach">🏗️ Embankment Breach</option><option value="other">📋 Other</option></select></div><button onClick={() => fileInputRef.current?.click?.()} className="w-full flex items-center justify-center gap-2 py-6 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-[#1a237e]">{reportPhoto ? <img src={reportPhoto} alt="" className="w-20 h-20 object-cover rounded" /> : <><Camera className="w-5 h-5" /><span className="text-sm">Upload photo</span></>}</button><input type="file" accept="image/*" capture="environment" onChange={e => { const f = e.target.files?.[0]; if(f) { const r = new FileReader(); r.onload = ev => setReportPhoto(ev.target?.result as string); r.readAsDataURL(f) } }} className="hidden" ref={fileInputRef} /><textarea value={reportDesc} onChange={e => setReportDesc(e.target.value)} placeholder="Describe the situation..." className="w-full px-3 py-2 border border-gray-300 rounded text-sm text-gray-800 placeholder-gray-400 focus:border-[#1a237e]" rows={3} /><div className="flex items-center gap-2 text-[10px] text-gray-400"><MapPin className="w-3 h-3" />GPS: {simLoc ? `${simLoc.lat.toFixed(4)}°N, ${simLoc.lon.toFixed(4)}°E (${simLoc.method})` : "detecting..."}</div></div><button onClick={handleSubmitReport} className="w-full py-2.5 bg-[#1a237e] text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2"><Send className="w-4 h-4" />Submit Report to NDRF</button></> : <div className="text-center space-y-4"><div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-green-100 border border-green-300"><Shield className="w-7 h-7 text-green-600" /></div><h2 className="text-lg font-bold text-green-800">{t(language, "report_submitted")}</h2><p className="text-sm text-gray-600">Report submitted from {userDistrict.name} to NDRF and local ward office. Visible in Authority Dashboard.</p><button onClick={() => { setReportSent(false); setReportPhoto(null); setReportDesc(""); setView("home") }} className="w-full py-2.5 bg-[#1a237e] text-white rounded-lg text-sm font-bold">{t(language, "back")}</button></div>}</div>
        </div>
    );

    // ─── ANALYTICS (HYDROMET) ───
    if(view === "analytics") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title="Hydrometeorological Analytics" showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
                ...
                {/* Rainfall Trend as simple bar chart */}
                <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">📈 Rainfall Trend (10 days)</h3>
                    <div className="flex items-end gap-1 h-24">{(liveRisk?.weather?.daily_precipitation || [12, 8, 25, 40, 62, 35, 18, 9, 15, 22]).slice(-10).map((v, i) => { const max = Math.max(...(liveRisk?.weather?.daily_precipitation || [62]).slice(-10)); return <div key={i} className="flex-1 flex flex-col items-center justify-end gap-0.5"><span className="text-[8px] text-gray-400">{v.toFixed ? v.toFixed(0) : v}</span><div className={`w-full rounded-t ${Number(v) > 40 ? "bg-red-500" : Number(v) > 20 ? "bg-yellow-500" : "bg-blue-400"}`} style={{ height: `${Math.max(4, (Number(v) / Math.max(max, 1)) * 80)}px` }} /></div> })}</div>
                    <div className="flex gap-1 mt-1 text-[8px] text-gray-400">{(liveRisk?.weather?.daily_dates || []).slice(-10).map((d, i) => <div key={i} className="flex-1 text-center truncate">{typeof d === 'string' ? d.slice(5) : ''}</div>)}</div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-2">🤖 Forecasting Model</h3>
                    <div className="space-y-2 text-xs text-gray-600"><p><strong>Model:</strong> {liveRisk?.model === "trained" ? "XGBoost ML" : "AI Analysis (Heuristic)"}</p><p><strong>Data Sources:</strong> Open-Meteo (Weather + Soil), GloFAS (River Discharge), IMD (Radar)</p><p><strong>Features:</strong> Rainfall intensity, accumulation, soil moisture, terrain, discharge, persistence</p><p><strong>Accuracy:</strong> ~85% for 1-hour forecasts, ~72% for 6-hour</p><p><strong>Zone Resolution:</strong> District-level with hyperlocal interpolation</p></div>
                </div>
                {/* Drainage / Infrastructure */}
                <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-2">🏗️ Infrastructure Status</h3>
    ...
    // ─── LOCATION INFO (SIM GPS) ───
    if(view === "location-info") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title="Your Location (SIM GPS)" showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4"><p className="text-xs text-blue-700"><strong>How it works:</strong> Like &quot;Where is my Train&quot;, your location is detected via mobile network cell tower triangulation. Works even without GPS or internet!</p></div>
                <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">📍 Current Location</h3>
                    <div className="grid grid-cols-2 gap-3 text-xs">{[{ l: "Latitude", v: simLoc ? `${simLoc.lat.toFixed(4)}°N` : "..." }, { l: "Longitude", v: simLoc ? `${simLoc.lon.toFixed(4)}°E` : "..." }, { l: "Accuracy", v: simLoc ? `±${simLoc.accuracy_m}m` : "..." }, { l: "Method", v: simLoc?.method === "gps" ? "GPS Satellite" : simLoc?.method === "cell_tower" ? "Cell Tower" : "Wi-Fi" }, { l: "Tower ID", v: simLoc?.tower_id || "..." }, { l: "Signal", v: simLoc?.signal_strength ? `${simLoc.signal_strength} dBm` : "..." }].map((m, i) => <div key={i} className="bg-gray-50 border border-gray-100 rounded p-3"><p className="text-[10px] text-gray-400">{m.l}</p><p className="text-sm font-bold text-gray-800 font-mono">{m.v}</p></div>)}</div>
                </div>
                {towers.length > 0 && <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">📡 Nearest Cell Towers</h3><div className="space-y-2">{towers.map((tw, i) => <div key={i} className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded px-3 py-2"><div><p className="text-xs font-mono text-gray-800">{tw.id}</p><p className="text-[10px] text-gray-400">{tw.lat.toFixed(4)}°N, {tw.lon.toFixed(4)}°E</p></div><div className="text-right"><p className="text-xs font-bold text-gray-700">{tw.signal} dBm</p><p className="text-[10px] text-gray-400">{tw.distance_km} km</p></div></div>)}</div></div>}
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-[10px] text-gray-500">Selected region: <strong>{userDistrict.name}, {userState.name}</strong> · Updates are personalized for this area.</div>
            </div>
        </div>
    );

    // ─── STATE ANALYSIS ───
    if(view === "state-analysis") return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={t(language, "state_analysis")} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-3">{STATES_DATA.map(st => { const su = getStateRiskSummary(st); return <button key={st.name} onClick={() => setSelectedState(st)} className={`w-full bg-white border rounded-lg p-4 text-left transition-colors ${st.name === userState.name ? "border-[#1a237e] border-2" : "border-gray-200 hover:border-[#1a237e]"}`}><div className="flex items-center justify-between"><div><h3 className="text-sm font-bold text-gray-800">{st.name}{st.name === userState.name ? " 📍" : ""}</h3><p className="text-xs text-gray-400 mt-0.5">{st.districts.length} districts</p></div><div className="flex items-center gap-2"><div className="text-right text-[10px]">{su.highCount > 0 && <span className="block text-red-600 font-bold">{su.highCount} High</span>}{su.moderateCount > 0 && <span className="block text-yellow-600">{su.moderateCount} Mod</span>}</div><ChevronRight className="w-4 h-4 text-gray-400" /></div></div></button> })}</div>
        </div>
    );

    // ─── DISTRICT DETAIL ───
    if(view === "district-detail" && selectedDistrict) return (
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader title={selectedDistrict.name} showBack />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4"><div className={`border rounded-lg p-4 ${selectedDistrict.riskLevel === "HIGH" ? "bg-red-50 border-red-200" : selectedDistrict.riskLevel === "MODERATE" ? "bg-yellow-50 border-yellow-200" : "bg-green-50 border-green-200"}`}><span className={`text-xs font-bold px-2 py-0.5 rounded border ${sev(selectedDistrict.riskLevel)}`}>{selectedDistrict.riskLevel}</span><h2 className="text-lg font-bold text-gray-800 mt-2">{selectedDistrict.name}</h2><p className="text-xs text-gray-500">{selectedDistrict.stateName}</p></div><div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">{t(language, "current_conditions")}</h3><div className="grid grid-cols-2 gap-3">{[{ l: "Rainfall", v: `${selectedDistrict.rainfall}mm` }, { l: "Water Level", v: `${selectedDistrict.waterLevel}m` }, { l: "Drainage", v: selectedDistrict.drainageHealth || "N/A" }, { l: "Embankment", v: selectedDistrict.embankmentRisk || "N/A" }].map((m, i) => <div key={i} className="bg-gray-50 border border-gray-100 rounded p-3"><p className="text-[10px] text-gray-400">{m.l}</p><p className="text-sm font-bold text-gray-800 mt-0.5">{m.v}</p></div>)}</div></div></div>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#f5f5f0]"><GovHeader />
            <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">{t(language, "quick_actions")}</h3>
                    <div className="grid grid-cols-2 gap-2">{[{ i: <Navigation className="w-4 h-4" />, l: t(language, "evacuation_route"), v: "evacuate" as View, c: "#1a237e" }, { i: <MapPin className="w-4 h-4" />, l: t(language, "nearby_shelters"), v: "shelters" as View, c: "#1a237e" }, { i: <Siren className="w-4 h-4" />, l: t(language, "send_sos"), v: "sos" as View, c: "#b71c1c" }, { i: <Bell className="w-4 h-4" />, l: t(language, "alert_family"), v: "alert-family" as View, c: "#1a237e" }, { i: <Camera className="w-4 h-4" />, l: t(language, "report_flood"), v: "report" as View, c: "#1a237e" }, { i: <TrendingUp className="w-4 h-4" />, l: "Hydromet Analytics", v: "analytics" as View, c: "#1a237e" }, { i: <Heart className="w-4 h-4" />, l: t(language, "vulnerable_areas"), v: "vulnerability" as View, c: "#b71c1c" }, { i: <Droplets className="w-4 h-4" />, l: t(language, "dam_monitoring"), v: "dams" as View, c: "#1a237e" }, { i: <BarChart3 className="w-4 h-4" />, l: t(language, "state_analysis"), v: "state-analysis" as View, c: "#1a237e" }, { i: <Radio className="w-4 h-4" />, l: "SIM Location", v: "location-info" as View, c: "#1a237e" }].map((it, i) => <button key={i} onClick={() => setView(it.v)} className="flex items-center gap-2.5 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-left hover:border-[#1a237e] group"><div className="w-8 h-8 rounded flex items-center justify-center text-white" style={{ backgroundColor: it.c }}>{it.i}</div><span className="text-xs font-semibold text-gray-700 group-hover:text-[#1a237e]">{it.l}</span></button>)}</div>
                </div>
                {/* Recent Reports from your state */}
                {myReports.length > 0 && <div className="bg-white border border-gray-200 rounded-lg p-4"><h3 className="text-xs font-bold text-gray-500 uppercase mb-3">📝 Recent Reports ({userState.name})</h3><div className="space-y-2">{myReports.slice(0, 3).map((r, i) => <div key={i} className="flex items-start gap-2 bg-gray-50 border border-gray-100 rounded p-2"><span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${sev(r.severity)}`}>{r.severity}</span><div className="flex-1"><p className="text-xs text-gray-700">{r.description.slice(0, 80)}...</p><p className="text-[10px] text-gray-400 mt-0.5">{r.district} · {new Date(r.submittedAt).toLocaleTimeString()} · 👍 {r.upvotes}</p></div></div>)}</div></div>}
                {/* Dam Alerts */}
                {dangerDams.length > 0 && <div className="bg-red-50 border border-red-200 rounded-lg p-4"><h3 className="text-xs font-bold text-red-700 uppercase mb-2 flex items-center gap-1"><Droplets className="w-3.5 h-3.5" />{t(language, "dam_monitoring")}</h3>{dangerDams.slice(0, 3).map((d, i) => <div key={i} className="flex items-center justify-between py-1.5 border-b border-red-100 last:border-0"><div><p className="text-xs font-semibold text-gray-800">{d.name}</p><p className="text-[10px] text-gray-500">{d.river}·{d.state}</p></div><span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${damC(d.status)}`}>{d.status} ({d.current_level_pct}%)</span></div>)}<button onClick={() => setView("dams")} className="mt-2 text-[10px] font-bold text-[#1a237e]">{t(language, "view_all")} →</button></div>}
                {/* Alerts */}
